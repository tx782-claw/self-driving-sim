"""
IMM (Interacting Multiple Model) — 交互多模型跟踪器 (P2 增强)
...
实现要点
--------
- 模型 1：CV (6 状态) - filterpy 内置
- 模型 2：CA (9 状态) - filterpy 内置
- Markov 转移矩阵 (2x2)：默认 diag=0.95（待在自身模型概率高）
- 每个模型独立 EKF 预测/更新
- 模型概率更新用似然 + Bayes
- 输出：模型概率加权混合的状态向量
- 接口与 EKFTrack 兼容：predict / update / miss / to_track / is_alive
...
def make_imm_transition_matrix(diag: float = 0.95) -> np.ndarray:
    """
    2x2 Markov 转移矩阵 (CV <-> CA)

    默认 diag=0.95：每个模型 95% 概率维持自身，2.5% 切到对方
    （保守的模型切换，CA 抖动场景避免频繁跳变）
    """
    if not 0.5 < diag < 1.0:
...
class IMMTrack:
    ...
    状态: 6 (CV) 与 9 (CA) 维度不同。
    混合时: 把 CA 的 9 维状态投影到 CV 的 6 维（位置+速度，加速度扔掉）；
            CV → CA 投影补 0 加速度。
    """
    def __init__(self, track_id: int, initial_pos: np.ndarray,
        ...
        self.transition = transition_matrix if transition_matrix is not None else make_imm_transition_matrix()
    ...
    def predict(self, dt: float = None):
        ...
        # IMM 状态交互（在 predict 之前）
        # 混合概率 c_j = sum_i (π_ij * μ_i)
        c = self.transition.T @ self.model_probs  # shape (2,)
        # 混合权重 w_ij = π_ij * μ_i / c_j
        # 但这里我们想混合"上一帧状态"输入到本帧的 predict，
        # 实际做法：先更新混合概率，再用混合概率计算初始状态

        # 简化：先 predict，再更新模型概率
    ...
    def update(self, det_pos: np.ndarray, sensor_id: str, timestamp: float,
        ...
        # 更新模型概率 (Bayes with likelihood)
        # likelihood_j = N(z; z_pred_j, S_j)
        # 实际简化：基于 innovation magnitude 估算
        try:
            S_cv = self.cv_kf.S
            innov_cv = self.cv_kf.y
            S_ca = self.ca_kf.S
            innov_ca = self.ca_kf.y
            # 创新向量维数 = 3
            lik_cv = self._gaussian_likelihood(innov_cv, S_cv)
            lik_ca = self._gaussian_likelihood(innov_ca, S_ca)
        except (AttributeError, np.linalg.LinAlgError):
            lik_cv, lik_ca = 1.0, 1.0

        # 模型预测概率 c_j = sum_i π_ij μ_i
        c = self.transition.T @ self.model_probs
        # 后验 μ_j = c_j * L_j / sum_k(c_k * L_k)
    ...
    def get_position(self) -> np.ndarray:
        """模型概率加权的位置"""
        return self.model_probs[0] * self.cv_kf.x[:3] + self.model_probs[1] * self.ca_kf.x[:3]

    def get_velocity(self) -> np.ndarray:
        return self.model_probs[0] * self.cv_kf.x[3:6] + self.model_probs[1] * self.ca_kf.x[3:6]
